    ...
    return (
        <div className="container">
            ...
            <form id="intention-form" onSubmit={encontrarMelhorPetshop}>
                ...
                <input type="number" id="caesPequenos" value={caesPequenos} onChange={e => setCaesPequenos(e.target.value)} required />

                <label htmlFor="caesGrandes">Quantidade de Cães Grandes:</label>
                <input type="number" id="caesGrandes" value={caesGrandes} onChange={e => setCaesGrandes(e.target.value)} required />

                <button type="submit">Consultar</button>
            </form>
            {melhorPetshop && <div>O melhor petshop é {melhorPetshop.nome} com um custo total de R${melhorPetshop.custoTotal}.</div>}
        </div>
    );
}
